...
import { getAllAtRiskStudents } from '../../data/dummyGroups';

function RiskAlertPage() {
  const [filter, setFilter] = useState('todas');
  const students = useMemo(() => getAllAtRiskStudents(), []);
      ...
      <div className="flex-1 px-4 py-5 sm:px-6 sm:py-6">
        ...
        {visible.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {visible.map((student) => (
              <RiskStudentCard key={`${student.groupId}-${student.name}`} student={student} />
            ))}
          </div>
        )}
      </div>
