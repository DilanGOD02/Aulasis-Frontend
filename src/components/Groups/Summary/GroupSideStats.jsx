...
function GroupSideStats({ nextClassSchedule, studentCount, avgGeneral }) {
  ...
  return (
    <div className="flex flex-1 min-w-[220px] flex-col gap-3.5">
      <div className="rounded-2xl border border-[#EEF1F6] bg-white px-[22px] py-5 shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
        ...
        <div className="my-2 text-[18px] font-extrabold text-[#0F172A]">{nextClassSchedule}</div>
        <button
          type="button"
      ...
      <StatCard icon="ph-trend-up" label="Promedio general" value={avgGeneral.toFixed(1)} />
    </div>
  );
}
